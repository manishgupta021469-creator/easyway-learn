(() => {
  ...
  async function processUpload(){
    ...
    if(files.length && API_BASE && serverToken()){
      try{
        ...
        if(items.length===files.length) toast(`OCR processed ${items.filter(x=>x.ocr==='server OCR').length}/${files.length} page(s)`);
      }catch(e){
  ...
  function editModal(){
    ...
    if(state.modal.type==='newSubject')return `<div class="modal-backdrop"><div class="modal"><div class="row between"><h2 class="section-title">Create Subject</h2><button class="btn ghost" onclick="closeModal()">Close</button></div><div class="field"><label>SUBJECT NAME</label><input id="newSubName" placeholder="Example: Physics" /></div><div class="field"><label>LANGUAGE</label><select id="newSubLang"><option>English</option><option>Hindi</option></select></div><button class="btn primary" onclick="createSubject()">Create Subject</button></div></div>`;
    if(state.modal.type==='newBook')return `<div class="modal-backdrop"><div class="modal"><div class="row between"><h2 class="section-title">Create Book</h2><button class="btn ghost" onclick="closeModal()">Close</button></div><div class="field"><label>BOOK TITLE</label><input id="newBookTitle" placeholder="Example: Physics — Class 11" /></div><div class="field"><label>CLASS</label><input id="newBookClass" placeholder="11" /></div><button class="btn primary" onclick="createBook()">Create Book</button></div></div>`;
