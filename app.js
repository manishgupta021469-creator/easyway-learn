(() => {
  ...
  // Robust modal controls: do not depend on inline onclick handlers, and never let an
  // active speech-recognition session interfere with modal inputs/buttons.
  document.addEventListener('click',e=>{
    const close=e.target.closest?.('[data-easyway-close-modal]');
    if(close){e.preventDefault();e.stopPropagation();closeModal();}
  },true);
  document.addEventListener('focusin',e=>{
    if(!state.modal)return;
    const el=e.target;
    if(el && /^(INPUT|TEXTAREA|SELECT)$/.test(el.tagName)) stopRecognition(false);
  },true);
