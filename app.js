(() => {
  ...
  function openRegister(){state.modal={type:'register'};render()}
  function openForgot(){state.modal={type:'forgot'};render()}
  ...
  async function registerStudent(){
    ...
    if(!/^[A-Z0-9][A-Z0-9_-]{3,29}$/.test(id)){toast('Student ID 4–30 characters का रखें: letters, numbers, - या _');return}
  ...
  // Registration buttons use both the normal handlers and a delegated fallback so mobile browsers
  // cannot lose the Create Permanent ID click when the login shell is re-rendered.
  document.addEventListener('click',e=>{
    const b=e.target?.closest?.('#createPermanentIdBtn,[data-easyway-register]');
    if(b){e.preventDefault();openRegister();}
  });
